let browser = mp.browsers.new('package://browser/status-bar/status-bar.html');
browser.active = true;
...
setInterval(() => {
	mp.voiceChat.muted = !mp.keys.isDown(key)
	browser.execute(`updateVoice(${mp.voiceChat.muted.toString()});`);
